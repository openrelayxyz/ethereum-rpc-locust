...
def rand_heavy_block():
    return random.randint(53981705, latestBlock)

def rand_archive_block():
    return random.randint(0, 53981705)
...
class WebsiteUser(HttpUser):
    def __init__(self, *args, **kwargs):
        ...
        self.archive_session_tx_hashes = [*tx_hashes]
        self.archive_session_senders = []
        self.archive_session_recipients = []
        self.archive_block_num = latestBlock
        self.archive_block_hex = block_number
        self.archive_block_hash = block_hash
    ...
    def light_transactionCount(self):
        ...
        if self.light_session_recipients + self.light_session_senders:
            print(method)
            data = {"jsonrpc":"2.0","method": f"{method}","params":[random.choice(self.light_session_recipients + self.light_session_senders), "pending"],"id":random.randint(0, 9999)}
            response = run_request(self, data, name=f"light_{method}")
    ...
    def light_feeHistory(self):
        ...
        data = {"jsonrpc":"2.0","method": f"{method}","params":[hex(random.randint(0, 10)), self.light_block_hex, sorted([random.random(), random.random(), random.random()])], "id":random.randint(0, 9999)}
        response = run_request(self, data, name=f"light_{method}")
    ...
    def heavy_transactionCount(self):
        method = "eth_getTransactionCount"
        if self.heavy_session_recipients + self.heavy_session_senders:
            print(method)
            data = {"jsonrpc":"2.0","method": f"{method}","params":[random.choice(self.heavy_session_recipients + self.heavy_session_senders), self.heavy_block_hex],"id":random.randint(0, 9999)}
            response = run_request(self, data, name=f"heavy_{method}")
    ...
    def heavy_feeHistory(self):
        method = "eth_feeHistory"
        print(method) ## TODO float params
        data = {"jsonrpc":"2.0","method": f"{method}","params":[hex(random.randint(0, 10)), self.heavy_block_hex, sorted([random.random(), random.random(), random.random()])], "id":random.randint(0, 9999)}
        response = run_request(self, data, name=f"heavy_{method}")
...
# archive calls
    ...
    @task(weight=weight_object["eth_getBlockByNumber"] * archive_coefficient)
    @task(1000)
    def archive_blockByNumber(self):
        method = "eth_getBlockByNumber"
        print(method)
        self.archive_block_num=rand_archive_block()
        self.archive_block_hex=hex(self.archive_block_num)
        data = {"jsonrpc":"2.0","method":f"{method}","params":[self.archive_block_hex, True],"id":random.randint(0, 9999)}
        response = run_request(self, data, name=f"archive_{method}")
        if self.archive_block_num % 64 == 0:
            self.archive_65block_hash = response.json().get('result')['hash']
        else:
            self.archive_block_hash = response.json().get('result')['hash']
        self.archive_block_hash = response.json().get('result')['hash']
        transaction_hashes = [tx['hash'] for tx in response.json().get('result')['transactions']]
        recepients = [tx['to'] for tx in response.json().get('result')['transactions']]
        senders = [tx['from'] for tx in response.json().get('result')['transactions']]
        self.archive_session_tx_hashes.extend(transaction_hashes)
        self.archive_session_recipients.extend(recepients)
        self.archive_session_senders.extend(senders)

    @task(weight=weight_object["eth_getBlockByHash"] * archive_coefficient)
    def archive_blockByHash(self):
        method = "eth_getBlockByHash"
        print(method)
        data = {"jsonrpc":"2.0","method": f"{method}","params":[self.archive_block_hash, True],"id":random.randint(0, 9999)}
        response = run_request(self, data, name=f"archive_{method}")

    @task(weight=weight_object["eth_getLogs"] * archive_coefficient)
    def archive_getLogs(self):
        method = "eth_getLogs"
        print(method + "heavy var 1")
        data = {"jsonrpc":"2.0", "method": "eth_getLogs", "params": [{"fromBlock": hex(self.archive_block_num - random.randint(0, 3)), "toBlock": hex(self.archive_block_num), "address": random.choice(token_addresses), "topics":["0xddf252ad1be2c89b69c2b068fc378daa952ba7f163c4a11628f55a4df523b3ef"]}],"id":random.randint(0, 9999)}
        response = run_request(self, data, name=f"archive_{method} v1")

    @task(weight=weight_object["eth_getLogs"] * archive_coefficient)
    def light_getLogs(self):
        method = "eth_getLogs"
        print(method + "heavy var 2")
        data = {"jsonrpc":"2.0", "method": "eth_getLogs", "params": [{"fromBlock": hex(self.archive_block_num), "toBlock": hex(self.archive_block_num)}],"id":random.randint(0, 9999)}
        response = run_request(self, data, name=f"archive_{method} v2")

    @task(weight=weight_object["eth_getLogs"] * archive_coefficient)
    def light_getLogs(self):
        method = "eth_getLogs"
        print(method + "heavy var 3")
        data = {"jsonrpc":"2.0", "method": "eth_getLogs", "params": [{"fromBlock": hex(self.archive_block_num), "toBlock": hex(self.archive_block_num + 1)}],"id":random.randint(0, 9999)}
        response = run_request(self, data, name=f"archive_{method} v3")

    @task(weight=weight_object["eth_getTransactionByHash"] * archive_coefficient)
    def archive_transactionByHash(self):
        method = "eth_getTransactionByHash"
        print(method)
        data = {"jsonrpc":"2.0","method": f"{method}","params":[random.choice(self.archive_session_tx_hashes)],"id":random.randint(0, 9999)}
        response = run_request(self, data, name=f"archive_{method}")

        data = {"jsonrpc":"2.0", "method": "eth_getTransactionByHash", "params": [random.choice(tx_hash_misses)]}
        response = run_request(self, data, name="light_eth_getTransactionByHash (miss)")

    @task(weight=weight_object["eth_getBlockTransactionCountByNumber"] * archive_coefficient)
    def archive_blockTransactionCountByNumber(self):
        print(len(self.archive_transaction_hashes))
        method = "eth_getBlockTransactionCountByNumber"
        print(method)
        data = {"jsonrpc":"2.0","method": f"{method}","params":[self.archive_block_hex],"id":random.randint(0, 9999)}
        print(self.archive_block_hex)
        response = run_request(self, data, name=f"archive_{method}")
        self.archive_txcount_and_number = (self.archive_block_hex, int(response.json().get("result", "0x0"), 16))

    @task(weight=weight_object["eth_getBlockTransactionCountByHash"] * archive_coefficient)
    def archive_blockTransactionCountByHash(self):
        print(len(self.archive_transaction_hashes))
        method = "eth_getBlockTransactionCountByHash"
        print(method)
        data = {"jsonrpc":"2.0","method": f"{method}","params":[self.archive_block_hash],"id":random.randint(0, 9999)}
        response = run_request(self, data, name=f"archive_{method}")
        self.archive_txcount_and_hash = (self.archive_block_hash, int(response.json().get("result", "0x0"), 16))

    @task(weight=weight_object["eth_getTransactionReceipt"] * archive_coefficient)
    def archive_transactionReceipt(self):
        method = "eth_getTransactionReceipt"
        print(method)
        data = {"jsonrpc":"2.0","method": f"{method}","params":[random.choice(self.archive_session_tx_hashes)],"id":random.randint(0, 9999)}
        response = run_request(self, data, name=f"archive_{method}")
    ...
    @task(weight=weight_object["eth_getTransactionCount"] * archive_coefficient)
    def archive_transactionCount(self):
        method = "eth_getTransactionCount"
        if self.archive_session_recipients + self.archive_session_senders:
            print(method)
            data = {"jsonrpc":"2.0","method": f"{method}","params":[random.choice(self.archive_session_recipients + self.archive_session_senders), self.archive_block_hex],"id":random.randint(0, 9999)}
            response = run_request(self, data, name=f"archive_{method}")

    @task(weight=weight_object["eth_getUncleCountByBlockNumber"] * archive_coefficient)
    def archive_uncleCountByBlockNumber(self):
        method = "eth_getUncleCountByBlockNumber"
        print(method)
        data = {"jsonrpc":"2.0","method": f"{method}","params":[self.archive_block_hex],"id":random.randint(0, 9999)}
        response = run_request(self, data, name=f"archive_{method}")

    @task(weight=weight_object["eth_getUncleCountByBlockHash"] * archive_coefficient)
    def archive_uncleCountByBlockHash(self):
        method = "eth_getUncleCountByBlockHash"
        print(method)
        data = {"jsonrpc":"2.0","method": f"{method}","params":[self.archive_block_hash],"id":random.randint(0, 9999)}
        response = run_request(self, data, name=f"archive_{method}")

    @task(weight=weight_object["eth_gasPrice"] * archive_coefficient)
    def archive_gasPrice(self):
        method = "eth_gasPrice"
        print(method)
        data = {"jsonrpc":"2.0","method": f"{method}","id":random.randint(0, 9999)}
        response = run_request(self, data, name=f"archive_{method}")

    @task(weight=weight_object["eth_maxPriorityFeePerGas"] * archive_coefficient)
    def archive_maxPriorityFeePerGas(self):
        method = "eth_maxPriorityFeePerGas"
        print(method)
        data = {"jsonrpc":"2.0","method": f"{method}","id":random.randint(0, 9999)}
        response = run_request(self, data, name=f"archive_{method}")

    @task(weight=weight_object["eth_feeHistory"] * archive_coefficient)
    def archive_feeHistory(self):
        method = "eth_feeHistory"
        print(method) ## TODO float params
        data = {"jsonrpc":"2.0","method": f"{method}","params":[hex(random.randint(0, 10)), self.archive_block_hex, sorted([random.random(), random.random(), random.random()])], "id":random.randint(0, 9999)}
        response = run_request(self, data, name=f"archive_{method}")

    ## flume name space methods

    @task(weight=weight_object["flume_getTransactionsBySender"] * archive_coefficient) ## default heavy
    def archive_transactionsBySender(self):
        method = "flume_getTransactionsBySender"
        print(method)
        data = {"jsonrpc":"2.0","method": f"{method}","params":[random.choice(self.archive_session_senders)], "id":random.randint(0, 9999)}
        response = run_request(self, data, name=f"archive_{method}")

    @task(weight=weight_object["flume_getTransactionsByRecipient"] * archive_coefficient) ## default heavy
    def archive_transactionsByRecipient(self):
        method = "flume_getTransactionsByRecipient"
        print(method)
        data = {"jsonrpc":"2.0","method": f"{method}","params":[random.choice(self.archive_session_recipients)], "id":random.randint(0, 9999)}
        response = run_request(self, data, name=f"archive_{method}")

    @task(weight=weight_object["flume_getTransactionsByParticipant"] * archive_coefficient) ## default heavy
    def archive_transactionsByParticipant(self):
        method = "flume_getTransactionsByParticipant"
        print(method)
        data = {"jsonrpc":"2.0","method": f"{method}","params":[random.choice(account_addresses)], "id":random.randint(0, 9999)}
        response = run_request(self, data, name=f"archive_{method}")

    @task(weight=weight_object["flume_getTransactionReceiptsBySender"] * archive_coefficient) ## default heavy
    def archive_transactionReceiptsBySender(self):
        method = "flume_getTransactionReceiptsBySender"
        print(method)
        data = {"jsonrpc":"2.0","method": f"{method}","params":[random.choice(self.archive_session_senders)], "id":random.randint(0, 9999)}
        response = run_request(self, data, name=f"archive_{method}")

    @task(weight=weight_object["flume_getTransactionReceiptsByRecipient"] * archive_coefficient) ## default heavy
    def archive_transactionReceiptsByRecipient(self):
        method = "flume_getTransactionReceiptsByRecipient"
        print(method)
        data = {"jsonrpc":"2.0","method": f"{method}","params":[random.choice(self.archive_session_recipients)], "id":random.randint(0, 9999)}
        response = run_request(self, data, name=f"archive_{method}")

    @task(weight=weight_object["flume_getTransactionReceiptsByParticipant"] * archive_coefficient) ## default heavy
    def archive_transactionReceiptsByParticipant(self):
        method = "flume_getTransactionReceiptsByParticipant"
        print(method)
        data = {"jsonrpc":"2.0","method": f"{method}","params":[random.choice(account_addresses)], "id":random.randint(0, 9999)}
        response = run_request(self, data, name=f"archive_{method}")
    ...
    @task(weight=weight_object["flume_getTransactionReceiptsByBlockNumber"] * archive_coefficient)
    def archive_transactionReceiptsByBlockNumber(self):
        method = "flume_getTransactionReceiptsByBlockNumber"
        print(method)
        data = {"jsonrpc":"2.0","method": f"{method}","params":[self.archive_block_hex], "id":random.randint(0, 9999)}
        response = run_request(self, data, name=f"archive_{method}")

    @task(weight=weight_object["flume_getTransactionReceiptsByBlockHash"] * archive_coefficient)
    def archive_transactionReceiptsByBlockHash(self):
        method = "flume_getTransactionReceiptsByBlockHash"
        print(method)
        data = {"jsonrpc":"2.0","method": f"{method}","params":[self.archive_block_hash], "id":random.randint(0, 9999)}
        response = run_request(self, data, name=f"archive_{method}")

    @task(weight=weight_object["flume_erc20ByAccount"] * archive_coefficient) ## default heavy
    def archive_eRC20ByAccount(self):
        method = "flume_erc20ByAccount"
        print(method)
        data = {"jsonrpc":"2.0","method": f"{method}","params":[random.choice(account_addresses)], "id":random.randint(0, 9999)}
        response = run_request(self, data, name=f"archive_{method}")

    @task(weight=weight_object["flume_erc20Holders"] * archive_coefficient) ## default heavy
    def archive_eRC20Holders(self):
        method = "flume_erc20Holders"
        print(method)
        data = {"jsonrpc":"2.0","method": f"{method}","params":[random.choice(account_addresses)], "id":random.randint(0, 9999)}
        response = run_request(self, data, name=f"archive_{method}")
    ...
    # @task(weight=weight_object["bor_getSnapshot"] * archive_coefficient)
    @task(weight=2000)
    def archive_snapshot(self):
        method = "bor_getSnapshot"
        print(method)
        data = {"jsonrpc":"2.0","method": f"{method}","params":[self.archive_block_hex], "id":random.randint(0, 9999)}
        response = run_request(self, data, name=f"archive_{method} block")

        data = {"jsonrpc":"2.0","method": f"{method}","params":[self.archive_block_hash], "id":random.randint(0, 9999)}
        response = run_request(self, data, name=f"archive_{method} hash")

    # @task(weight=weight_object["bor_getAuthor"] * archive_coefficient)
    @task(weight=2000)
    def archive_author(self):
        method = "bor_getAuthor"
        print(method)
        data = {"jsonrpc":"2.0","method": f"{method}","params":[self.archive_block_hex], "id":random.randint(0, 9999)}
        response = run_request(self, data, name=f"archive_{method}")

    # @task(weight=weight_object["bor_getRootHash"] * archive_coefficient)
    @task(weight=2000)
    def archive_rootHash(self):
        method = "bor_getRootHash"
        print(method)
        data = {"jsonrpc":"2.0","method": f"{method}","params":[(self.archive_block_num - random.randint(0, 1000)), self.archive_block_num], "id":random.randint(0, 9999)}
        response = run_request(self, data, name=f"archive_{method}")

    @task(weight=weight_object["bor_getSignersAtHash"] * archive_coefficient)
    @task(weight=2000)
    def archive_getSignersAtHash(self):
        method = "bor_getSignersAtHash"
        print(method)

        if len(self.archive_64block_hash) > 0:
            data = {"jsonrpc":"2.0","method": f"{method}","params":[self.archive_64block_hash], "id":random.randint(0, 9999)}
            response = run_request(self, data, name=f"archive_{method}")
        else:
            data = {"jsonrpc":"2.0","method": f"{method}","params":[self.archive_block_hash], "id":random.randint(0, 9999)}
            response = run_request(self, data, name=f"archive_{method} error")

    # @task(weight=weight_object["bor_getCurrentValidators"] * archive_coefficient)
    @task(weight=2000)
    def archive_currentValidators(self):
        method = "bor_getCurrentValidators"
        print(method)
        data = {"jsonrpc":"2.0","method": f"{method}","id":random.randint(0, 9999)}
        response = run_request(self, data, name=f"archive_{method}")

    # @task(weight=weight_object["bor_getCurrentProposer"] * archive_coefficient)
    @task(weight=2000)
    def archive_currentProposer(self):
        method = "bor_getCurrentProposer"
        print(method)
        data = {"jsonrpc":"2.0","method": f"{method}","id":random.randint(0, 9999)}
        response = run_request(self, data, name=f"archive_{method}")
    ...
    # @task(weight=weight_object["eth_getBorBlockReceipt"] * archive_coefficient)
    @task(weight=2000)
    def archive_borBlockReceipt(self):
        method = "eth_getBorBlockReceipt"
        print(method)
    ...
    # @task(weight=weight_object["eth_getTransactionReceiptsByBlock"] * archive_coefficient)
    @task(weight=2000)
    def archive_transactionReceiptsByBlock(self):
        method = "eth_getTransactionReceiptsByBlock"
        print(method)
